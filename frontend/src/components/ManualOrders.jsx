      ...
      {showPayDueModal && payingOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl flex flex-col">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              <h3 className="text-lg font-bold text-slate-800">Collect Outstanding Due</h3>
              <button onClick={() => { setShowPayDueModal(false); setPayingOrder(null); }} className="text-slate-400 hover:text-slate-650">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            ...
            <form onSubmit={handlePayDueSubmit} className="mt-4 space-y-4">
              <div>
                <p className="text-xs text-slate-500 mb-1">Customer Profile Name</p>
                <p className="text-sm font-bold text-slate-800 bg-slate-50 p-2.5 border border-slate-100 rounded-xl">{payingOrder.customer_name}</p>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Payment Amount *</label>
                <input
                  type="number"
                  step="0.01"
                  value={payDueAmount}
                  max={parseFloat(payingOrder.current_sale_due).toFixed(2)}
                  onChange={(e) => setPayDueAmount(e.target.value)}
                  required
                  className="w-full border border-slate-200 rounded-lg p-2.5 text-sm outline-none focus:ring-1 focus:ring-indigo-500"
                />
                <span className="text-[10px] text-slate-450 mt-1 block">Maximum remaining due amount: ৳{parseFloat(payingOrder.current_sale_due).toFixed(2)}</span>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Payment Type Method *</label>
                <select
                  value={payDueMethod}
                ...
                >
                  ...
                  <option value="mobile_pay">Mobile Financial Pay (BKash/Nagad/Rocket)</option>
              ...
              <div className="pt-4 border-t border-slate-100 flex space-x-3 justify-end">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-semibold transition-colors shadow disabled:opacity-50"
                >
                  {payDueSubmitting ? 'Recording...' : 'Record Payment'}
                </button>
              </div>
            </form>
          </div>
